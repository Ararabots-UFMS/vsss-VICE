...
class DefensePosition(LeafNode):
    ...
    def run(self):
        ...
        if self.check_for_ball_in_defense_area():
            return (
                TaskStatus.SUCCESS,
                self.movement.move_to_position_with_orientation_no_obstacle(
                    self.ball.position_x, self.ball.position_y, theta
                ),
            )
        else:
            return (
    ...
    def check_for_ball_in_defense_area(self):
        ...
        if self.penalty_area.is_colission([self.ball.position_x, self.ball.position_y]):
            ...
            if (
                self.blackboard.gui.is_field_side_left
                and self.blackboard.balls[0].position_x < 0
            ):
                return True
            elif (
                not self.blackboard.gui.is_field_side_left
                and self.blackboard.balls[0].position_x > 0
            ):
                return True
    ...
    def calculate_distance_to_ball_goal(self):
        return math.sqrt(
            (self.ball.position_x - self.penalty_stretch_x) ** 2
            + (self.ball.position_y) ** 2
        )
    ...
    def draw_line(self, id):
        ...
        m = (self.robot_y - self.ball.position_y) / (
            self.robot_x - self.ball.position_x
        )
        b = self.ball.position_y - m * self.ball.position_x

        theta = math.atan2(
            self.robot_y - self.ball.position_y, self.robot_x - self.ball.position_x
        )
    ...
    def closest_enemy_with_ball(self):
        ...
        #         (enemy_robots[enemy].position_x - self.ball.position_x) ** 2
        #         + (enemy_robots[enemy].position_y - self.ball.position_y) ** 2
        #     )
...
class CheckBallDistance(LeafNode):
    ...
    def run(self):
        ...
        distance = math.sqrt(
            (self.goalkeeper.position_x - self.ball_position_x) ** 2
            + (self.goalkeeper.position_y - self.ball_position_y) ** 2
        )
