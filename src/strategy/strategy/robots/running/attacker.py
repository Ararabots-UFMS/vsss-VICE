...
class MoveToBall(LeafNode):
    ...
    def run(self):
        ...
        return TaskStatus.SUCCESS, self.movement.moveToBall()
    ...
    def draw_line(self):
        ...
        if self.blackboard.gui.is_field_side_left:
            ...
            theta = (
                math.atan2((-self.ball_y), (self.goal_field.x1 - self.ball_x)) + math.pi
            )
...
class CheckBallDistance(LeafNode):
    ...
    def run(self):
        ...
        distance = math.sqrt(
            (self.position_x - self.ball_position_x) ** 2
            + (self.position_y - self.ball_position_y) ** 2
        )
...
class CheckGoalDistance(LeafNode):
    ...
    def run(self):
        ...
        distance = math.sqrt(
            (self.position_x - self.goal_position_x) ** 2
            + (self.position_y - self.goal_position_y) ** 2
        )
...
class MoveToGoal(LeafNode):
    ...
    def run(self):
        ...
        if self.blackboard.gui.is_field_side_left:
            ...
            return TaskStatus.SUCCESS, self.movement.moveToEnemyGoal(
                self.goal_position_x, self.goal_position_y, self.theta
            )
        else:
            # self.blackboard.activate_kick()
            self.theta = math.pi
