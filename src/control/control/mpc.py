...
class Controller():
    def __init__(self, max_velocity, max_angular_vel, n_horizon = 2, t_step = 0.1, n_robust = 0):
        ...
        self.mpc = get_mpc(max_velocity, max_angular_vel, n_horizon = n_horizon, t_step = t_step, n_robust = n_robust, store_full_solution = False)
    ...
    def tvp_func(self, t_now):
        for k in range(self.n_horizon):
            ...
            orientation, _, _ = self.orientation_trajectory.at_time(t_now + k*self.t_step)
...
def get_mpc(max_velocity, max_angular_vel, n_horizon, t_step, n_robust, store_full_solution):
    model = get_model()
    mpc = MPC(model)
    ...
    lterm = (model.x['x'] - model.tvp['ref_x'])**2 + (model.x['y'] - model.tvp['ref_y'])**2 + (model.x['orientation'] - model.tvp['ref_orientation']) **2
    mterm = lterm
